...
public class ClienteControllerV2 {
    ...
    public CollectionModel<EntityModel<Cliente>> getClientes(){
        List<EntityModel<Cliente>> cliente = clienteService.getClientes().stream()
                .map(assembler::toModel)
                .collect(Collectors.toList());

        return CollectionModel.of(cliente,
                linkTo(methodOn(ClienteControllerV2.class).getClientes()).withSelfRel());
    }
