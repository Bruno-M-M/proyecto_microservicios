...
public class CarritoService {
    ...
    public CarritoDetalleDTO agregar(CarritoRequestDTO request) {
        ...
        // Mínimo 2 ítems (refuerzo en servicio, además de la anotación @Size del DTO)
        if (request.getItems() == null || request.getItems().size() < 2) {
            throw new RuntimeException("El pedido debe contener al menos 2 productos distintos.");
        }
        ...
        if (distintos < request.getItems().size()) {
            throw new RuntimeException(
                    "No puedes agregar el mismo producto más de una vez en el pedido. " +
