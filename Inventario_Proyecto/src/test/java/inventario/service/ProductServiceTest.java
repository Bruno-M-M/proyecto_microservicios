...
class ProductServiceTest {
    ...
    @Test
    void getProductByCategory_deberiaRetornarProductosDeEsaCategoria() {
        when(productRepository.findByCategoria("Lacteos")).thenReturn(List.of(product));
        when(mapper.toResponse(product)).thenReturn(responseDTO);

        List<ProductResponseDTO> resultado = productService.getProductByCategory("Lacteos");

        assertEquals(1, resultado.size());
        assertEquals(Product.Categorias.Lacteos, resultado.get(0).getCategoria());
    }

    @Test
    void getProductByCategory_cuandoNoHayProductos_deberiaRetornarListaVacia() {
        when(productRepository.findByCategoria("Carnes")).thenReturn(List.of());

        List<ProductResponseDTO> resultado = productService.getProductByCategory("Carnes");
