...
public class BoletaService {
    ...
    public BoletaResponseDTO emitirBoleta(BoletaRequestDTO request) {
        ...
        for (CarritoDetalleDTO pedido : pedidosConfirmados) {
            try {
                ...
                log.warn(" No se pudo marcar como PAGADO el pedido {}: {}", pedido.getPedidoId(), e.getMessage());
            }
        }
    ...
    public void notificarPedidoConfirmado(Long clienteId, Long pedidoId) {
        try {
            log.info(" Notificación recibida - Pedido {} del cliente {} confirmado y listo para pago",
                    pedidoId, clienteId);

        } catch (Exception e) {
            log.error("Error al procesar notificación de confirmación del pedido {}: {}", pedidoId, e.getMessage());
            throw new RuntimeException("Error interno al procesar notificación de pedido");
        }
    }
}